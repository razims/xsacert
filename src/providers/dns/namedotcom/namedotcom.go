...
func NewDefaultConfig() *Config {
	return &Config{
		...
		PropagationTimeout: env.GetOrDefaultSecond(EnvPropagationTimeout, 60*2*time.Minute),
...
func NewDNSProvider(username string, aPIToken string, server string) (*DNSProvider, error) {
...
func (d *DNSProvider) Present(domain, token, keyAuth string) error {
	...
	u, err2 := publicsuffix.Parse(fqdn)
	if err2 != nil {
		log.Fatal(err2)
	}

	request := &namecom.Record{
		DomainName: fmt.Sprintf("%s.%s", u.SLD, u.TLD),
		Host:       u.TRD,
		Type:       "TXT",
		Fqdn:       fqdn,
